...
async function clickLatestRun(page) {
  const selector = 'table tbody tr:first-child a';
  ...
  try {
    ...
    await page.evaluate((sel) => {
      ...
      if (!el) throw new Error('Latest DAG run link not found');
      el.click();
    }, selector);
  }
}
...
(async () => {
  ...
  try {
    ...
    await page.goto(`${baseUrl}/dagrun/list/?_flt_3_dag_id=${dagId}`, {
      waitUntil: 'domcontentloaded',
      timeout: 60000,
    });
    await page.waitForSelector('table', { timeout: 60000 });
    ...
    for (let attempt = 0; attempt < 3; attempt += 1) {
      try {
        runLink = await page.evaluate(() => {
          const link = document.querySelector('a[href*="dag_run_id"]');
    ...
    if (!runLink) {
      throw new Error('Latest DAG run link not found');
    }
    const runUrl = new URL(`${baseUrl}${runLink}`);
    const dagRunId = runUrl.searchParams.get('dag_run_id');
    if (!dagRunId) {
      throw new Error('dag_run_id not found in run link');
    }
    ...
    await page.waitForSelector('pre, code, .log-content', { timeout: 120000 });
    await delay(2000);
    ...
    console.error('Airflow incremental log verification failed:', err);
    process.exit(1);
  } finally {
    await browser.close();
  }
