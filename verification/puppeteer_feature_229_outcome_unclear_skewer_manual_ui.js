...
const SCREENSHOT_NAME =
  ...
  'feature-229-outcome-unclear-skewer-2026-01-31.png';
...
(async () => {
  ...
  try {
    ...
    const tactics = Array.isArray(dashboardPayload?.tactics)
      ...
      : [];
    const hasUnclear = tactics.some(
      (row) => row.motif === 'skewer' && row.result === 'unclear',
    );

    if (!hasUnclear) {
      throw new Error('Expected skewer tactic with unclear outcome');
    }
    ...
    const hasTableRow = await page.evaluate(() => {
      ...
      return rows.some((row) => {
        ...
        return text.includes('unclear') && text.includes('skewer');
      });
    });
    if (!hasTableRow) {
      throw new Error('Recent tactics table missing unclear skewer row');
    }
