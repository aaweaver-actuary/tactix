...
console.log('Starting feature 176 Puppeteer run...');
...
const SCREENSHOT_NAME =
  ...
  'feature-176-hanging-piece-blitz-low-severity-ci-2026-01-29.png';
...
(async () => {
  ...
  try {
      ...
      async (apiBase, expectedGameId, maxSeverity) => {
        ...
        return Array.isArray(data?.tactics)
          ? data.tactics.some((row) => {
              ...
              if (expectedGameId && row.game_id !== expectedGameId)
                return false;
    ...
    if (!severityCheck) {
      throw new Error(
        'Expected hanging piece tactic with low severity (<= 1.0)',
      );
    }
