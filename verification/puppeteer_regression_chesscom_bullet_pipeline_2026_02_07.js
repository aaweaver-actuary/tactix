...
(async () => {
  ...
  try {
    ...
    await setSelectValue(page, 'filter-source', 'chesscom');
    await setSelectValue(page, 'filter-chesscom-profile', 'bullet');
    await setSelectValue(page, 'filter-time-control', 'bullet');
    await setDateInput(page, 'filter-start-date', '2026-02-01');
    await setDateInput(page, 'filter-end-date', '2026-02-01');
    ...
    const outPath = await captureScreenshot(
      page,
      'regression-chesscom-bullet-pipeline-2026-02-07.png',
    );
    ...
    try {
      const outPath = await captureScreenshot(
        page,
        'failed-regression-chesscom-bullet-pipeline-2026-02-07.png',
      );
