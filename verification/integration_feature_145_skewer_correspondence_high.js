...
(async () => {
  try {
    ...
    const row = tactics.find(
      (item) =>
        item.motif === 'skewer' &&
        item.game_id === 'correspondence-skewer-high' &&
        Number(item.severity) >= 1.5,
    );
    if (!row) {
    ...
    console.log('Feature 145 integration check ok');
  } catch (err) {
    console.error('Feature 145 integration check failed:', err);
    process.exit(1);
  }
})();
