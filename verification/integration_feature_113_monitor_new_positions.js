...
(async () => {
  ...
  const allowed = new Set(['*/10 * * * *', '0/10 * * * *']);
  if (!allowed.has(scheduleValue)) {
    throw new Error(`Expected 10-minute schedule, got: ${scheduleValue}`);
  }
  ...
  console.log('Monitor DAG schedule and task checks passed.');
})();
