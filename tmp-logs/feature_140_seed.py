...
def _skewer_fixture_position() -> tuple[dict[str, object], str]:
    fixture_path = Path("tests/fixtures/skewer.pgn")
    ...
    for chunk in chunks:
        ...
        return (
            {
                "game_id": f"{extract_game_id(chunk)}-correspondence",
                "user": "chesscom",
                "source": "chesscom",
                "fen": board.fen(),
                "ply": board.ply(),
    ...
    raise SystemExit("No skewer fixture position found")
...
position, pgn_chunk = _skewer_fixture_position()

conn = get_connection(Path("data") / "tactix.duckdb")
init_schema(conn)
if "TimeControl" not in pgn_chunk:
    lines = pgn_chunk.splitlines()
    header_index = next(
        (idx for idx, line in enumerate(lines) if not line.startswith("[")),
        len(lines),
    )
    lines.insert(header_index, '[TimeControl "86400"]')
    pgn_chunk = "\n".join(lines).strip() + "\n"
upsert_raw_pgns(
    conn,
    [
...
row = conn.execute(
    ...
    LIMIT 1
    """,
    [position["uci"], position["fen"], position["game_id"]],
).fetchone()
...
if result is None:
    raise SystemExit("No tactic result for skewer correspondence fixture")
...
print("seeded skewer correspondence tactic into data/tactix.duckdb")
