...
console.log('Starting feature 122 Puppeteer run...');
...
const SCREENSHOT_NAME =
  ...
  'feature-122-fork-classical-high-severity-ci-2026-01-28.png';
  ...
  try {
    ...
    const severityCheck = await page.evaluate(async (apiBase) => {
      const res = await fetch(
        `${apiBase}/api/dashboard?source=chesscom&motif=fork&time_control=1800`,
        {
    ...
    if (!forkRow.best_san || !forkRow.explanation) {
      throw new Error('Expected best line and explanation to be persisted');
    }

    if (!forkRow.position_id) {
      throw new Error('Expected tactic to be linked to a position');
    }
