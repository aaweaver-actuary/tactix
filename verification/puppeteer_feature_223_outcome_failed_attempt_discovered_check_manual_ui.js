...
const SCREENSHOT_NAME =
  ...
  'feature-223-outcome-failed-attempt-discovered-check-2026-01-31.png';
  ...
  try {
    ...
    const hasTableRow = await page.evaluate(() => {
      ...
      return rows.some((row) => {
        ...
        return (
          text.includes('failed_attempt') && text.includes('discovered_check')
        );
      });
    });
    if (!hasTableRow) {
      throw new Error(
        'Recent tactics table missing failed_attempt discovered_check row',
      );
    }
