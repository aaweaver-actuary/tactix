...
def _discovered_attack_fixture_position() -> dict[str, object]:
    ...
    raise AssertionError("No discovered attack classical fixture position found")
...
def _discovered_attack_high_fixture_position() -> dict[str, object]:
    ...
    side_to_move = "white" if board.turn == chess.WHITE else "black"
    return {
        "game_id": "classical-discovered-attack-high",
        "user": "chesscom",
        "source": "chesscom",
        "fen": board.fen(),
        "ply": board.ply(),
...
def _discovered_attack_correspondence_fixture_position() -> dict[str, object]:
    ...
    raise AssertionError("No discovered attack correspondence fixture position found")


class DiscoveredAttackClassicalTests(unittest.TestCase):
    @unittest.skipUnless(shutil.which("stockfish"), "Stockfish binary not on PATH")
    def test_classical_discovered_attack_is_low_severity(self) -> None:
        settings = Settings(
            source="chesscom",
        ...
        conn = get_connection(tmp_dir / "discovered_attack_classical.duckdb")
        init_schema(conn)
    ...
    @unittest.skipUnless(shutil.which("stockfish"), "Stockfish binary not on PATH")
    def test_classical_discovered_attack_is_high_severity(self) -> None:
        settings = Settings(
            source="chesscom",
            chesscom_user="chesscom",
            chesscom_profile="classical",
        ...
        tmp_dir = Path(tempfile.mkdtemp())
        conn = get_connection(tmp_dir / "discovered_attack_classical_high.duckdb")
        init_schema(conn)
        position_ids = insert_positions(conn, [position])
        position["position_id"] = position_ids[0]
        ...
        tactic_row, outcome_row = result
        self.assertEqual(tactic_row["motif"], "discovered_attack")
        self.assertEqual(tactic_row["game_id"], "classical-discovered-attack-high")
    ...
    @unittest.skipUnless(shutil.which("stockfish"), "Stockfish binary not on PATH")
    def test_classical_discovered_attack_records_missed_outcome(self) -> None:
        settings = Settings(
            source="chesscom",
            chesscom_user="chesscom",
            chesscom_profile="classical",
        ...
        tmp_dir = Path(tempfile.mkdtemp())
        conn = get_connection(tmp_dir / "discovered_attack_classical_missed.duckdb")
        init_schema(conn)

        with StockfishEngine(settings) as engine:
            missed_position, result = find_missed_position(
                position, engine, settings, "discovered_attack"
            )
    ...
    @unittest.skipUnless(shutil.which("stockfish"), "Stockfish binary not on PATH")
    def test_classical_discovered_attack_records_failed_attempt_outcome(self) -> None:
        settings = Settings(
            source="chesscom",
            chesscom_user="chesscom",
            chesscom_profile="classical",
...
class TestDiscoveredAttackCorrespondence(unittest.TestCase):
    ...
    def test_correspondence_discovered_attack_is_low_severity(self) -> None:
        ...
        tmp_dir = Path(tempfile.mkdtemp())
        conn = get_connection(tmp_dir / "discovered_attack_correspondence.duckdb")
        init_schema(conn)
        position_ids = insert_positions(conn, [position])
        position["position_id"] = position_ids[0]
