...
console.log('Starting feature 174 Puppeteer run...');
...
const SCREENSHOT_NAME =
  ...
  'feature-174-hanging-piece-bullet-low-severity-ci-2026-01-29.png';
const EXPECTED_GAME_ID =
  process.env.TACTIX_HANGING_PIECE_GAME_ID || 'bullet-hanging-piece-low';
...
(async () => {
  ...
  try {
    ...
    const severityCheck = await page.evaluate(
      async (apiBase, expectedGameId, maxSeverity) => {
        ...
        return Array.isArray(data?.tactics)
          ? data.tactics.some((row) => {
              if (row.motif !== 'hanging_piece') return false;
    ...
    if (!severityCheck) {
      throw new Error('Expected hanging piece tactic with low severity (<= 1.0)');
    }
