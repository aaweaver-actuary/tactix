...
const screenshotName =
  ...
  'feature-unclear-tactical-puzzle-2026-02-10.png';
...
(async () => {
  ...
  try {
    ...
    const bestLabel = await page.$eval(
      '[data-testid="practice-best-move"]',
      (node) => (node.textContent || '').trim(),
    );
    if (!bestLabel || bestLabel.includes('Best --')) {
      throw new Error(
        `Best move label was missing for the practice puzzle: "${bestLabel}"`,
      );
    }
    ...
    console.error('Unclear puzzle verification failed:', err);
    process.exit(1);
  } finally {
    await browser.close();
  }
