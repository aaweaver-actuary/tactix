...
console.log('Starting feature 078 Puppeteer run...');
...
const SCREENSHOT_NAME =
  ...
  'feature-078-mate-in-1-bullet-high-severity-2026-01-26.png';
...
(async () => {
  ...
  try {
    ...
    const severityCheck = await page.evaluate(async () => {
      const res = await fetch(
        '/api/dashboard?source=chesscom&motif=mate&time_control=60',
        {
    ...
    const hasHighSeverity = Array.isArray(severityCheck?.tactics)
      ? severityCheck.tactics.some(
          (row) => row.best_uci === 'd8h4' && row.severity >= 1.5,
        )
      : false;

    if (!hasHighSeverity) {
      throw new Error('Expected mate-in-one tactic with high severity (>= 1.5)');
    }
