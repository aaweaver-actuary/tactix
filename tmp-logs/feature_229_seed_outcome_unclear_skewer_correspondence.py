...
def _build_unclear_result(
    ...
    if result is None:
        raise SystemExit("No result for skewer unclear seed position")
    tactic_row, outcome_row = result
    if tactic_row["motif"] != "skewer" or outcome_row["result"] != "unclear":
        raise SystemExit("Expected unclear skewer outcome for seed position")
...
def _skewer_fixture_positions() -> list[dict[str, object]]:
    fixture_path = Path("tests/fixtures/skewer.pgn")
    chunks = split_pgn_chunks(fixture_path.read_text())
    positions: list[dict[str, object]] = []
    for idx, chunk in enumerate(chunks, start=1):
        position = _first_move_position(chunk, game_id=f"skewer-unclear-{idx}")
        if position:
...
positions = _skewer_fixture_positions()
if not positions:
    raise SystemExit("No skewer fixture positions found")
...
print("seeded skewer (correspondence) unclear outcome into data/tactix.duckdb")
