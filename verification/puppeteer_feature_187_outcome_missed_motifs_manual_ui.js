...
const SCREENSHOT_NAME =
  ...
  'feature-187-outcome-missed-motifs-manual-2026-01-29.png';
...
(async () => {
  ...
  try {
    ...
    const checks = [
      ...
      {
        label: 'discovered attack (classical)',
        motif: 'discovered_attack',
        profile: 'classical',
      },
      {
        label: 'discovered check (blitz)',
    ...
    for (const check of checks) {
      ...
      const hasMissed = await tableHasResult(table, 'missed');
      if (!hasMissed) {
        throw new Error(`Expected missed outcome rows for ${check.label}`);
      }
    }
