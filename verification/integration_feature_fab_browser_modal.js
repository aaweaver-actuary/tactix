...
const selectors = {
  ...
  browserOpen: '[data-testid="browser-open"]',
...
(async () => {
  ...
  try {
    ...
    const rowText = await page.evaluate((selector) => {
      const row = document.querySelector(selector);
      return row ? row.textContent || '' : '';
    }, selectors.row);

    const parsedFen = extractFenFromRow(rowText);
    if (!parsedFen) {
    ...
    await page.waitForSelector(selectors.browserOpen, { timeout: 60000 });
    await page.click(selectors.browserOpen);

    await page.waitForSelector(selectors.modal, { timeout: 60000 });
    await page.waitForSelector(selectors.board, { timeout: 60000 });

    const modalState = await page.evaluate((sel) => {
      const modal = document.querySelector(sel.modal);
      const boardContainer = document.querySelector(sel.board);
      const closeButton = document.querySelector(sel.close);
    ...
    if (
      ...
      throw new Error('Browser modal did not render expected controls.');
    }

    if (!modalState.boardHasTexture) {
      throw new Error('Browser modal board did not render listudy styling.');
    }

    if (!modalState.modalText.includes(parsedFen)) {
      throw new Error('Browser modal did not include the expected FEN.');
    }
    ...
    console.log('Browser modal FAB integration verified.');
  } finally {
    await browser.close();
  }
})().catch((err) => {
