...
class ChesscomClient(BaseChessClient):
    ...
    def _use_fixture_games(self) -> bool:
        ...
        return bool(
            not self.settings.chesscom_token
            and self.settings.chesscom_use_fixture_when_no_token
        )
    ...
    def _fetch_remote_games(self, since_ms: int, full_history: bool) -> list[dict]:
        ...
        archives = self._fetch_archive_index(since_ms)
        if archives is None:
    ...
    def _fetch_archive_index(self, since_ms: int) -> list[str] | None:
        ...
        try:
            ...
            self.logger.warning(
                "Falling back to fixtures; archive fetch failed: %s", exc
            )
            return None
    ...
    def _get_with_backoff(self, url: str, timeout: int) -> requests.Response:
        ...
        while True:
            ...
            if response.status_code != 429:
                ...
                return response
            attempt = self._handle_rate_limit(
                response, attempt, max_retries, base_backoff
            )
    ...
    def _attach_cursors(self, games: list[dict]) -> list[dict]:
    ...
    def _resolve_last_timestamp(self, games: list[dict], cursor: str | None) -> int:
    ...
    def _resolve_next_cursor(self, games: list[dict], cursor: str | None) -> str | None:
    ...
    def _log_fetch_summary(
        self, cursor: str | None, next_cursor: str | None, count: int
    ) -> None:
...
def _next_page_url(data: dict, current_url: str) -> str | None:
    ...
    for key in ("next_page", "next", "next_url", "nextPage"):
        ...
        if isinstance(candidate, dict):
            ...
            if href:
                return href
    return _page_from_numbers(data, current_url)
...
def _page_from_numbers(data: dict, current_url: str) -> str | None:
    ...
    if page is None or total_pages is None or page >= total_pages:
        return None
...
def fetch_incremental_games(
    ...
    return cast(
        ChesscomFetchResult, ChesscomClient(context).fetch_incremental_games(request)
    )
