...
def _skewer_classical_high_position() -> dict[str, object]:
    fixture_path = Path("tests/fixtures/chesscom_classical_sample.pgn")
    chunks = split_pgn_chunks(fixture_path.read_text())
    skewer_pgn = next(chunk for chunk in chunks if "Classical Fixture 6" in chunk)
    game = chess.pgn.read_game(StringIO(skewer_pgn))
    if not game:
        raise SystemExit("No skewer classical high game found")
    fen = game.headers.get("FEN")
    board = chess.Board(fen) if fen else game.board()
    moves = list(game.mainline_moves())
    if not moves:
        raise SystemExit("No skewer classical high moves found")
    move = moves[0]
...
position = _skewer_classical_high_position()
...
row = conn.execute(
    ...
    WHERE source = 'chesscom'
      AND game_id = 'classical-skewer-high'
      AND uci = ?
      AND fen = ?
...
if result is None:
    raise SystemExit("No tactic result for skewer classical high fixture")
...
if tactic_row["motif"] != "skewer":
    raise SystemExit("Expected skewer motif for classical high fixture")
if tactic_row["severity"] < 1.5:
    raise SystemExit("Expected high severity (>= 1.5) for classical skewer")

upsert_tactic_with_outcome(conn, tactic_row, outcome_row)
print("seeded skewer classical high tactic into data/tactix.duckdb")
