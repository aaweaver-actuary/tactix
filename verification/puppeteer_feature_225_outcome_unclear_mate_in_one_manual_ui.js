...
const SCREENSHOT_NAME =
  ...
  'feature-225-outcome-unclear-mate-in-one-2026-01-31.png';
...
(async () => {
  ...
  try {
    ...
    const hasUnclear = await page.evaluate(async (apiBase) => {
      ...
      return tactics.some(
        (row) => row.result === 'unclear' && row.best_uci === 'd8h4',
      );
    }, API_BASE);

    if (!hasUnclear) {
      throw new Error(
        'Expected mate-in-one tactic with unclear outcome for best_uci d8h4',
      );
    }
    ...
    const hasTableRow = await page.evaluate(() => {
      ...
      return rows.some((row) => {
        ...
        return text.includes('unclear') && text.includes('mate');
      });
    });
    if (!hasTableRow) {
      throw new Error('Recent tactics table missing unclear mate row');
    }
