...
async function fetchDashboard() {
  const url = `${apiBase}/api/dashboard?source=chesscom&motif=pin&time_control=86400`;
...
(async () => {
  try {
    ...
    console.log('Feature 135 integration check ok');
  } catch (err) {
    console.error('Feature 135 integration check failed:', err);
    process.exit(1);
  }
})();
