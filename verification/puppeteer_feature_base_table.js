...
const SCREENSHOT_NAME =
  ...
  `feature-base-table-${SOURCE}-2026-01-29.png`;
...
async function clickButtonByLabel(page, label) {
  const xpath = `//button[contains(normalize-space(.), "${label}")]`;
...
(async () => {
  ...
  try {
    ...
    if (SOURCE === 'chesscom') {
      await clickButtonByLabel(page, 'Chess.com');
    }

    await clickButtonByLabel(page, ACTION_LABEL);
