...
async function fetchTactics() {
  const url = `${apiBase}/api/tactics/search?source=all&limit=5&motif=fork`;
...
(async () => {
  try {
    ...
    console.log('Feature 235 manual check ok');
  } catch (err) {
    console.error('Feature 235 manual check failed:', err);
    process.exit(1);
  }
})();
