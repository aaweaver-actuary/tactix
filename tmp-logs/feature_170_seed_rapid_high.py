...
def _discovered_check_fixture_position(fixture_path: Path) -> dict[str, object]:
    ...
    target_event = (os.getenv("TACTIX_DISCOVERED_CHECK_EVENT") or "discovered check high").lower()
    for chunk in chunks:
    ...
    raise SystemExit("No discovered check high fixture position found")


profile = os.getenv("TACTIX_CHESSCOM_PROFILE", "rapid")
...
if result is None:
    raise SystemExit("No tactic result for discovered check high fixture")
...
print("seeded discovered check high tactic into data/tactix.duckdb")
