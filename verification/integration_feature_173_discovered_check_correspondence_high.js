...
async function fetchDashboard() {
  const url = `${apiBase}/api/dashboard?source=chesscom&motif=discovered_check&time_control=86400`;
...
(async () => {
  try {
    ...
    console.log('Feature 173 integration check ok');
  } catch (err) {
    console.error('Feature 173 integration check failed:', err);
    process.exit(1);
  }
})();
