...
const SCREENSHOT_NAME =
  ...
  'feature-230-outcome-unclear-discovered-attack-2026-01-31.png';
...
(async () => {
  ...
  try {
    ...
    const hasUnclear = tactics.some(
      (row) => row.motif === 'discovered_attack' && row.result === 'unclear',
    );
    ...
    const hasTableRow = await page.evaluate(() => {
      ...
      return rows.some((row) => {
        ...
        return text.includes('unclear') && text.includes('discovered_attack');
      });
    });
    if (!hasTableRow) {
      throw new Error('Recent tactics table missing unclear discovered attack row');
    }
