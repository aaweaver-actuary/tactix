...
def _pin_fixture_position() -> tuple[dict[str, object], str]:
    ...
    raise SystemExit("No pin fixture position found")
...
position, pgn_chunk = _pin_fixture_position()
...
if result is None:
    raise SystemExit("No tactic result for pin correspondence fixture")
...
if tactic_row["severity"] < 1.5:
    raise SystemExit("Expected high severity (>= 1.5) for correspondence pin")

upsert_tactic_with_outcome(conn, tactic_row, outcome_row)
print("seeded pin correspondence high severity tactic into data/tactix.duckdb")
