...
const SCREENSHOT_NAME =
  ...
  'feature-220-outcome-failed-attempt-pin-2026-01-31.png';
...
(async () => {
  ...
  try {
    ...
    const hasFailedAttempt = tactics.some(
      (row) => row.motif === 'pin' && row.result === 'failed_attempt',
    );
    ...
    const hasTableRow = await page.evaluate(() => {
      ...
      return rows.some((row) => {
        ...
        return text.includes('failed_attempt') && text.includes('pin');
      });
    });
    if (!hasTableRow) {
      throw new Error('Recent tactics table missing failed_attempt pin row');
    }
