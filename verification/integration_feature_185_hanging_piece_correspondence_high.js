...
(async () => {
  try {
    ...
    const row = tactics.find(
      (item) =>
        item.motif === 'hanging_piece' &&
        item.game_id === 'correspondence-hanging-piece-high' &&
    ...
    console.log('Feature 185 integration check ok');
  } catch (err) {
    console.error('Feature 185 integration check failed:', err);
    process.exit(1);
  }
})();
