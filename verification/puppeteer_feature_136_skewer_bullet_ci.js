...
console.log('Starting feature 136 Puppeteer run...');
...
const SCREENSHOT_NAME =
  ...
  'feature-136-skewer-bullet-low-severity-ci-2026-01-28.png';
...
(async () => {
  ...
  try {
    ...
    const hasSkewerRow = rows.some((row) => row.toLowerCase().includes('skewer'));
    if (!hasSkewerRow) {
    ...
    const severityCheck = await page.evaluate(async (apiBase) => {
      const res = await fetch(
        `${apiBase}/api/dashboard?source=chesscom&motif=skewer`,
        {
    ...
    const hasLowSeverity = Array.isArray(severityCheck?.tactics)
      ? severityCheck.tactics.some(
          (row) => row.motif === 'skewer' && row.severity <= 1.0,
        )
      : false;
