...
def _hanging_piece_fixture_position() -> dict[str, object]:
    ...
    hanging_chunk = next(
        chunk for chunk in chunks if "Correspondence Fixture 11 - Hanging Piece Low" in chunk
    )
    ...
    return {
        "game_id": "correspondence-hanging-piece-low",
        "user": "chesscom",
        "source": "chesscom",
        "fen": board.fen(),
        "ply": board.ply(),
...
if result is None:
    raise SystemExit("No tactic result for hanging piece correspondence fixture")
...
print("seeded hanging piece correspondence tactic into data/tactix.duckdb")
