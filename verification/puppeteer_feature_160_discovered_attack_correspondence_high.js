...
const SCREENSHOT_NAME =
  ...
  'feature-160-discovered-attack-correspondence-high-severity-2026-01-28.png';
const GAME_ID =
  process.env.TACTIX_DISCOVERED_ATTACK_GAME_ID ||
  'correspondence-discovered-attack-high';
...
(async () => {
  ...
  try {
    ...
    const hasHighSeverity = await page.evaluate(
      async (apiBase, gameId, minSeverity) => {
        ...
        return Array.isArray(data?.tactics)
          ? data.tactics.some(
              (row) =>
                ...
                row.game_id === gameId &&
                Number(row.severity) >= minSeverity,
            )
          : false;
      },
      API_BASE,
    ...
    if (!hasHighSeverity) {
      throw new Error('Expected discovered attack tactic with high severity');
    }
