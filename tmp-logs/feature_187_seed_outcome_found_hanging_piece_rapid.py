...
hanging_position = positions[0]

conn = get_connection(Path("data") / "tactix.duckdb")
init_schema(conn)

with StockfishEngine(settings) as engine:
    result = analyze_position(hanging_position, engine, settings=settings)

if result is None:
    raise SystemExit("No tactic result for hanging piece rapid fixture")

tactic_row, outcome_row = result
if outcome_row["result"] != "found" and tactic_row.get("best_uci"):
    hanging_position["uci"] = tactic_row["best_uci"]
    try:
        board = chess.Board(str(hanging_position["fen"]))
        move = chess.Move.from_uci(hanging_position["uci"])
        hanging_position["san"] = board.san(move)
    except Exception:
        pass
    with StockfishEngine(settings) as engine:
        result = analyze_position(hanging_position, engine, settings=settings)
    if result is None:
        raise SystemExit("No tactic result after forcing hanging piece to best move")
    tactic_row, outcome_row = result

if outcome_row["result"] != "found":
    raise SystemExit("Expected found outcome for hanging piece rapid fixture")

hanging_position = _ensure_position(conn, hanging_position)
tactic_row["position_id"] = hanging_position["position_id"]

upsert_tactic_with_outcome(conn, tactic_row, outcome_row)
update_metrics_summary(conn)
write_metrics_version(conn)
print("seeded hanging piece (rapid) found outcome into data/tactix.duckdb")
