...
const SCREENSHOT_NAME =
  ...
  'feature-221-outcome-failed-attempt-skewer-2026-01-31.png';
...
async function fetch_dashboard__payload(apiBase) {
  const res = await fetch(`${apiBase}/api/dashboard?source=chesscom&motif=skewer`, {
...
(async () => {
  ...
  try {
    ...
    const hasFailedAttempt = tactics.some(
      (row) => row.motif === 'skewer' && row.result === 'failed_attempt',
    );

    if (!hasFailedAttempt) {
      throw new Error('Expected skewer tactic with failed_attempt outcome');
    }
    ...
    const hasTableRow = await page.evaluate(() => {
      ...
      return rows.some((row) => {
        ...
        return text.includes('failed_attempt') && text.includes('skewer');
      });
    });
    if (!hasTableRow) {
      throw new Error('Recent tactics table missing failed_attempt skewer row');
    }
