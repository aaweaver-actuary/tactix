...
console.log('Starting feature 096 Puppeteer run...');
...
const SCREENSHOT_NAME =
  ...
  'feature-096-fork-classical-low-severity-ci-2026-01-26.png';
  ...
  try {
    ...
    const hasLowSeverity = Array.isArray(severityCheck?.tactics)
      ? severityCheck.tactics.some(
          (row) => row.best_uci === 'f4e2' && row.severity <= 1.0,
        )
      : false;

    if (!hasLowSeverity) {
      throw new Error('Expected fork tactic with low severity (<= 1.0)');
    }
