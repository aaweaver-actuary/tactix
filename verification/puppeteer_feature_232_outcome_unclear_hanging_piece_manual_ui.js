...
const SCREENSHOT_NAME =
  ...
  'feature-232-outcome-unclear-hanging-piece-2026-01-31.png';
...
(async () => {
  ...
  try {
    ...
    const hasUnclear = tactics.some(
      (row) => row.motif === 'hanging_piece' && row.result === 'unclear',
    );
    ...
    const hasTableRow = await page.evaluate(() => {
      ...
      return rows.some((row) => {
        ...
        return text.includes('unclear') && text.includes('hanging_piece');
      });
    });
    if (!hasTableRow) {
      throw new Error('Recent tactics table missing unclear hanging_piece row');
    }
