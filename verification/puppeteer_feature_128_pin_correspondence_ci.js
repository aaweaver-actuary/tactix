...
console.log('Starting feature 128 Puppeteer run...');
...
const SCREENSHOT_NAME =
  ...
  'feature-128-pin-correspondence-low-severity-ci-2026-01-28.png';
...
(async () => {
  ...
  try {
    ...
    await page.select('[data-testid="filter-motif"]', 'pin');
    ...
    const hasPinRow = rows.some((row) => row.toLowerCase().includes('pin'));
    if (!hasPinRow) {
      throw new Error('Expected a pin row in tactics table');
    }

    const severityCheck = await page.evaluate(async (apiBase) => {
      const res = await fetch(
        `${apiBase}/api/dashboard?source=chesscom&motif=pin&time_control=86400`,
        {
    ...
    const hasLowSeverity = Array.isArray(severityCheck?.tactics)
      ? severityCheck.tactics.some(
          (row) => row.motif === 'pin' && row.severity <= 1.0,
        )
      : false;

    if (!hasLowSeverity) {
      throw new Error('Expected pin tactic with low severity (<= 1.0)');
    }
