...
def _discovered_attack_fixture_position() -> dict[str, object]:
    fixture_path = Path(__file__).resolve().parent / "fixtures" / "discoveredattack.pgn"
...
class DiscoveredAttackBulletTests(unittest.TestCase):
    @unittest.skipUnless(shutil.which("stockfish"), "Stockfish binary not on PATH")
    def test_bullet_discovered_attack_is_low_severity(self) -> None:
        settings = Settings(
            source="chesscom",
        ...
        conn = get_connection(tmp_dir / "discovered_attack_bullet.duckdb")
        init_schema(conn)
    ...
    @unittest.skipUnless(shutil.which("stockfish"), "Stockfish binary not on PATH")
    def test_bullet_discovered_attack_is_high_severity(self) -> None:
        settings = Settings(
            source="chesscom",
            chesscom_user="chesscom",
            chesscom_profile="bullet",
        ...
        tmp_dir = Path(tempfile.mkdtemp())
        conn = get_connection(tmp_dir / "discovered_attack_bullet_high.duckdb")
        init_schema(conn)
        position_ids = insert_positions(conn, [position])
        position["position_id"] = position_ids[0]
        ...
        tactic_row, outcome_row = result
        self.assertEqual(tactic_row["motif"], "discovered_attack")
        self.assertEqual(tactic_row["game_id"], "bullet-discovered-attack-high")
