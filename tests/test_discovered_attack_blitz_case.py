...
def _discovered_attack_fixture_position() -> dict[str, object]:
    ...
    raise AssertionError("No discovered attack blitz fixture position found")
...
def _discovered_attack_high_fixture_position() -> dict[str, object]:
    ...
    side_to_move = "white" if board.turn == chess.WHITE else "black"
    return {
        "game_id": "blitz-discovered-attack-high",
        "user": "chesscom",
        "source": "chesscom",
        "fen": board.fen(),
        "ply": board.ply(),
...
class DiscoveredAttackBlitzTests(unittest.TestCase):
    @unittest.skipUnless(shutil.which("stockfish"), "Stockfish binary not on PATH")
    def test_blitz_discovered_attack_is_low_severity(self) -> None:
        settings = Settings(
            source="chesscom",
        ...
        conn = get_connection(tmp_dir / "discovered_attack_blitz.duckdb")
        init_schema(conn)
    ...
    @unittest.skipUnless(shutil.which("stockfish"), "Stockfish binary not on PATH")
    def test_blitz_discovered_attack_is_high_severity(self) -> None:
        settings = Settings(
            source="chesscom",
            chesscom_user="chesscom",
            chesscom_profile="blitz",
        ...
        tmp_dir = Path(tempfile.mkdtemp())
        conn = get_connection(tmp_dir / "discovered_attack_blitz_high.duckdb")
        init_schema(conn)
        position_ids = insert_positions(conn, [position])
        position["position_id"] = position_ids[0]
        ...
        tactic_row, outcome_row = result
        self.assertEqual(tactic_row["motif"], "discovered_attack")
        self.assertEqual(tactic_row["game_id"], "blitz-discovered-attack-high")
