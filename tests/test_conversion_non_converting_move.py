...
class ConversionNonConvertingMoveTests(unittest.TestCase):
    @unittest.skipUnless(shutil.which("stockfish"), "Stockfish binary not on PATH")
    def test_non_converting_move_marks_conversion_false(self) -> None:
        settings = build_settings__chesscom_blitz_stockfish()

        fen = "4k3/8/8/8/3q4/5N2/8/4K3 w - - 0 1"
        board = chess.Board(fen)
        user_move = chess.Move.from_uci("f3g1")

        self.assertFalse(board.is_capture(user_move))
        self.assertIn(user_move, board.legal_moves)

        position = {
            "game_id": "conversion-non-converting-move",
            "user": "chesscom",
            "source": "chesscom",
            "fen": fen,
            "ply": board.ply(),
        ...
        conn = create_connection__conversion("conversion_non_converting_move.duckdb")
        ...
        self.assertFalse(conversion[0])
        self.assertIsNone(conversion[1])
        self.assertIn(conversion[2], {"missed", "failed_attempt", "unclear"})
