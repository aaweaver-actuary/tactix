...
(async () => {
  ...
  try {
    ...
    const attempted = await startPracticeAttempt(page, ['lichess', 'chesscom']);
    if (!attempted) {
      console.log('No practice items available; skipping attempt validation.');
    }
    ...
    console.log('MVP step-by-step integration verified.');
  } finally {
    await browser.close();
  }
})().catch((err) => {
