...
class SkewerCorrespondenceTests(unittest.TestCase):
    ...
    def test_correspondence_skewer_is_high_severity(self) -> None:
        ...
        tmp_dir = Path(tempfile.mkdtemp())
        conn = get_connection(tmp_dir / "skewer_correspondence_high.duckdb")
        init_schema(conn)
        ...
        tactic_row, outcome_row = result
        self.assertEqual(tactic_row["motif"], "skewer")
        self.assertEqual(tactic_row["game_id"], "correspondence-skewer-high")
        self.assertGreaterEqual(tactic_row["severity"], 1.5)
        self.assertLessEqual(outcome_row["eval_delta"], -150)
        self.assertTrue(tactic_row["best_uci"])

    @unittest.skipUnless(shutil.which("stockfish"), "Stockfish binary not on PATH")
    def test_correspondence_skewer_records_missed_outcome(self) -> None:
        settings = Settings(
            source="chesscom",
            chesscom_user="chesscom",
            chesscom_profile="correspondence",
