...
const SCREENSHOT_NAME =
  ...
  'feature-227-outcome-unclear-fork-2026-01-31.png';
  ...
  try {
    ...
    const hasUnclear = await page.evaluate(async (apiBase) => {
      const res = await fetch(
        `${apiBase}/api/dashboard?source=chesscom&motif=fork`,
        {
      ...
      return tactics.some(
        (row) => row.result === 'unclear' && row.motif === 'fork',
      );
    }, API_BASE);
    ...
    const hasTableRow = await page.evaluate(() => {
      ...
      return rows.some((row) => {
        ...
        return text.includes('unclear') && text.includes('fork');
      });
    });
    if (!hasTableRow) {
      throw new Error('Recent tactics table missing unclear fork row');
    }
