...
base_board.set_piece_at(chess.parse_square("h7"), chess.Piece(chess.KING, chess.BLACK))
base_board.set_piece_at(chess.parse_square("d8"), chess.Piece(chess.QUEEN, chess.BLACK))
base_board.set_piece_at(chess.parse_square("e7"), chess.Piece(chess.BISHOP, chess.WHITE))
...
with StockfishEngine(settings) as engine:
    for square in chess.SQUARES:
        if square in [
            chess.parse_square("h7"),
            chess.parse_square("d8"),
        ...
        for move in board.legal_moves:
            ...
            if tactic_row["motif"] != "hanging_piece" or outcome_row["result"] != "missed":
                continue
            if move.from_square != chess.parse_square("h7"):
