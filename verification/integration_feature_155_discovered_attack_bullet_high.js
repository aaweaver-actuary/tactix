  ...
  try {
    ...
    const row = tactics.find(
      (item) =>
        ...
        item.game_id === 'bullet-discovered-attack-high' &&
        Number(item.severity) >= 1.5,
    );
    if (!row) {
      throw new Error(
        'Expected discovered attack tactic with high severity (>= 1.5)',
      );
    }
    ...
    console.log('Feature 155 integration check ok');
  } catch (err) {
    console.error('Feature 155 integration check failed:', err);
    process.exit(1);
  }
})();
