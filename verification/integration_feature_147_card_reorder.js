...
(async () => {
  ...
  try {
    ...
    if (!storedOrder.includes('practice-queue')) {
      throw new Error('Stored card order missing practice-queue');
    }

    console.log('Feature 147 integration check ok');
  } catch (err) {
    console.error('Feature 147 integration check failed:', err);
    process.exit(1);
  } finally {
    if (browser) {
      await browser.close();
