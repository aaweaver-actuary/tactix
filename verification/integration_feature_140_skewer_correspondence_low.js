...
async function fetchDashboard() {
  const url = `${apiBase}/api/dashboard?source=chesscom&motif=skewer&time_control=86400`;
  ...
  try {
    ...
    const row = tactics.find(
      (item) =>
        ...
        item.game_id.endsWith('-correspondence') &&
        Number(item.severity) <= 1.0,
    );
    if (!row) {
      throw new Error('Expected skewer tactic with low severity (<= 1.0)');
    }
    if (!row.best_uci || typeof row.best_uci !== 'string') {
      throw new Error('Expected best_uci to be persisted for skewer tactic');
    }
    ...
    console.log('Feature 140 integration check ok');
  } catch (err) {
    console.error('Feature 140 integration check failed:', err);
    process.exit(1);
  }
})();
