...
if result is None:
    raise SystemExit("No tactic result for pin fixture")
...
if outcome_row["result"] != "found" and tactic_row.get("best_uci"):
    ...
    with StockfishEngine(settings) as engine:
        result = analyze_position(position, engine, settings=settings)
    if result is None:
        raise SystemExit("No tactic result after forcing pin to best move")
    tactic_row, outcome_row = result

if outcome_row["result"] != "found":
    raise SystemExit("Expected found outcome for pin correspondence fixture")
...
print("seeded pin (correspondence) outcome into data/tactix.duckdb")
