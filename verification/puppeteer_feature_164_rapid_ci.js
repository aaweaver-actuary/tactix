...
console.log('Starting feature 164 Puppeteer run...');
const BACKEND_CMD = path.join(ROOT_DIR, '.venv', 'bin', 'python');
const DUCKDB_PATH =
  process.env.TACTIX_DUCKDB_PATH ||
  path.join(ROOT_DIR, 'data', 'tactix_feature_164.duckdb');
const CHECKPOINT_PATH =
  process.env.TACTIX_CHESSCOM_CHECKPOINT_PATH ||
  path.join(ROOT_DIR, 'data', 'chesscom_feature_164_cursor.txt');
...
const SCREENSHOT_NAME =
  ...
  'feature-164-discovered-check-rapid-low-severity-ci-2026-01-28.png';
const EXPECTED_GAME_ID =
  process.env.TACTIX_DISCOVERED_CHECK_GAME_ID || '6677889907';
      ...
      async (apiBase, expectedGameId) => {
        ...
        return Array.isArray(data?.tactics)
          ? data.tactics.some((row) => {
              ...
              return Number(row.severity) <= 1.0;
            })
          : false;
      },
      API_BASE,
