...
def _skewer_high_fixture_position() -> dict[str, object]:
    ...
    skewer_chunk = next(chunk for chunk in chunks if "Blitz Fixture 6" in chunk)
...
class SkewerBlitzTests(unittest.TestCase):
    ...
    @unittest.skipUnless(shutil.which("stockfish"), "Stockfish binary not on PATH")
    def test_blitz_skewer_is_high_severity(self) -> None:
        settings = Settings(
            source="chesscom",
            chesscom_user="chesscom",
            chesscom_profile="blitz",
        ...
        tmp_dir = Path(tempfile.mkdtemp())
        conn = get_connection(tmp_dir / "skewer_blitz_high.duckdb")
        init_schema(conn)
