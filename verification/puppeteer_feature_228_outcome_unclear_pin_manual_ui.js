...
const SCREENSHOT_NAME =
  ...
  'feature-228-outcome-unclear-pin-2026-01-31.png';
...
(async () => {
  ...
  try {
    ...
    const hasUnclear = tactics.some(
      (row) => row.motif === 'pin' && row.result === 'unclear',
    );

    if (!hasUnclear) {
      throw new Error('Expected pin tactic with unclear outcome');
    }
    ...
    const hasTableRow = await page.evaluate(() => {
      ...
      return rows.some((row) => {
        ...
        return text.includes('unclear') && text.includes('pin');
      });
    });
    if (!hasTableRow) {
      throw new Error('Recent tactics table missing unclear pin row');
    }
