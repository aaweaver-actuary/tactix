...
process.env.TACTIX_SCREENSHOT_NAME =
  process.env.TACTIX_SCREENSHOT_NAME ||
  'feature-154-discovered-attack-correspondence-low-severity-2026-01-28.png';

require('./puppeteer_feature_152_discovered_attack_classical_manual');
  process.env.TACTIX_CHESSCOM_PROFILE || 'correspondence';
		const proc = spawn(
			BACKEND_CMD,
			[
...
function isPortOpen(host, port, timeoutMs = 1000) {
	return new Promise((resolve) => {
		...
		const finalize = (result) => {
			if (settled) return;			if (settled) return;			socket.destroy();
			resolve(result);
		};
