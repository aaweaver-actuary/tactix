...
def _discovered_check_fixture_position() -> dict[str, object]:
    ...
    raise AssertionError("No discovered check blitz fixture position found")
...
def _discovered_check_high_fixture_position() -> dict[str, object]:
    ...
    moves = list(game.mainline_moves())
    if not moves:
        raise AssertionError("No moves in discovered check high fixture")
    move = moves[0]
    side_to_move = "white" if board.turn == chess.WHITE else "black"
    return {
...
def _discovered_check_rapid_fixture_position() -> dict[str, object]:
    ...
    raise AssertionError("No discovered check rapid fixture position found")
...
def _discovered_check_rapid_high_fixture_position() -> dict[str, object]:
    ...
    moves = list(game.mainline_moves())
    if not moves:
        raise AssertionError("No moves in discovered check rapid high fixture")
    move = moves[0]
    side_to_move = "white" if board.turn == chess.WHITE else "black"
    return {
        "game_id": "rapid-discovered-check-high",
        "user": "chesscom",
        "source": "chesscom",
        "fen": board.fen(),
        "ply": board.ply(),
...
def _discovered_check_classical_fixture_position() -> dict[str, object]:
    ...
    raise AssertionError("No discovered check classical fixture position found")
...
def _discovered_check_classical_high_fixture_position() -> dict[str, object]:
    ...
    moves = list(game.mainline_moves())
    if not moves:
        raise AssertionError("No moves in discovered check classical high fixture")
    move = moves[0]
    side_to_move = "white" if board.turn == chess.WHITE else "black"
    return {
        "game_id": "classical-discovered-check-high",
        "user": "chesscom",
        "source": "chesscom",
        "fen": board.fen(),
        "ply": board.ply(),
...
def _discovered_check_correspondence_fixture_position() -> dict[str, object]:
    ...
    raise AssertionError("No discovered check correspondence fixture position found")
...
def _discovered_check_correspondence_high_fixture_position() -> dict[str, object]:
    ...
    moves = list(game.mainline_moves())
    if not moves:
        raise AssertionError("No moves in discovered check correspondence high fixture")
    move = moves[0]
    side_to_move = "white" if board.turn == chess.WHITE else "black"
    return {
        "game_id": "correspondence-discovered-check-high",
        "user": "chesscom",
        "source": "chesscom",
        "fen": board.fen(),
        "ply": board.ply(),
...
def _discovered_check_unit_fixture_position() -> dict[str, object]:
    fixture_path = Path(__file__).resolve().parent / "fixtures" / "discoveredcheck.pgn"
    chunks = split_pgn_chunks(fixture_path.read_text())
    if not chunks:
        raise AssertionError("No discovered check fixture chunks found")
    game = chess.pgn.read_game(StringIO(chunks[0]))
    if not game:
        raise AssertionError("No discovered check fixture game found")
    fen = game.headers.get("FEN")
    board = chess.Board(fen) if fen else game.board()
    moves = list(game.mainline_moves())
    if not moves:
        raise AssertionError("No moves in discovered check fixture")
    move = moves[0]
    side_to_move = "white" if board.turn == chess.WHITE else "black"
    return {
        "game_id": extract_game_id(chunks[0]),
        "user": "chesscom",
        "source": "chesscom",
        "fen": board.fen(),
        "ply": board.ply(),
...
class DiscoveredCheckBlitzTests(unittest.TestCase):
    @unittest.skipUnless(shutil.which("stockfish"), "Stockfish binary not on PATH")
    def test_blitz_discovered_check_is_low_severity(self) -> None:
        settings = Settings(
            source="chesscom",
        ...
        conn = get_connection(tmp_dir / "discovered_check_blitz.duckdb")
        init_schema(conn)
    ...
    @unittest.skipUnless(shutil.which("stockfish"), "Stockfish binary not on PATH")
    def test_blitz_discovered_check_records_found_outcome(self) -> None:
        settings = Settings(
            source="chesscom",
            chesscom_user="chesscom",
            chesscom_profile="blitz",
        ...
        position = _discovered_check_unit_fixture_position()

        tmp_dir = Path(tempfile.mkdtemp())
        conn = get_connection(tmp_dir / "discovered_check_found.duckdb")
        init_schema(conn)
        position_ids = insert_positions(conn, [position])
        position["position_id"] = position_ids[0]
    ...
    @unittest.skipUnless(shutil.which("stockfish"), "Stockfish binary not on PATH")
    def test_blitz_discovered_check_records_missed_outcome(self) -> None:
        settings = Settings(
            source="chesscom",
            chesscom_user="chesscom",
            chesscom_profile="blitz",
        ...
        tmp_dir = Path(tempfile.mkdtemp())
        conn = get_connection(tmp_dir / "discovered_check_blitz_missed.duckdb")
        init_schema(conn)
    ...
    def test_blitz_discovered_check_is_high_severity(self) -> None:
        ...
        tmp_dir = Path(tempfile.mkdtemp())
        conn = get_connection(tmp_dir / "discovered_check_blitz_high.duckdb")
        init_schema(conn)
        position_ids = insert_positions(conn, [position])
        position["position_id"] = position_ids[0]
        ...
        tactic_row, outcome_row = result
        self.assertEqual(tactic_row["motif"], "discovered_check")
        self.assertEqual(tactic_row["game_id"], "blitz-discovered-check-high")
    ...
    @unittest.skipUnless(shutil.which("stockfish"), "Stockfish binary not on PATH")
    def test_rapid_discovered_check_is_low_severity(self) -> None:
        settings = Settings(
            source="chesscom",
            chesscom_user="chesscom",
        ...
        position = _discovered_check_rapid_fixture_position()

        tmp_dir = Path(tempfile.mkdtemp())
        conn = get_connection(tmp_dir / "discovered_check_rapid.duckdb")
        init_schema(conn)
        position_ids = insert_positions(conn, [position])
        position["position_id"] = position_ids[0]
    ...
    @unittest.skipUnless(shutil.which("stockfish"), "Stockfish binary not on PATH")
    def test_rapid_discovered_check_is_high_severity(self) -> None:
        settings = Settings(
            source="chesscom",
            chesscom_user="chesscom",
            chesscom_profile="rapid",
        ...
        position = _discovered_check_rapid_high_fixture_position()

        tmp_dir = Path(tempfile.mkdtemp())
        conn = get_connection(tmp_dir / "discovered_check_rapid_high.duckdb")
        init_schema(conn)
        position_ids = insert_positions(conn, [position])
        position["position_id"] = position_ids[0]
        ...
        tactic_row, outcome_row = result
        self.assertEqual(tactic_row["motif"], "discovered_check")
        self.assertEqual(tactic_row["game_id"], "rapid-discovered-check-high")
        self.assertGreaterEqual(tactic_row["severity"], 1.5)
        self.assertLessEqual(outcome_row["eval_delta"], -150)
        self.assertTrue(tactic_row["best_uci"])
    ...
    @unittest.skipUnless(shutil.which("stockfish"), "Stockfish binary not on PATH")
    def test_classical_discovered_check_is_low_severity(self) -> None:
        settings = Settings(
            source="chesscom",
            chesscom_user="chesscom",
        ...
        position = _discovered_check_classical_fixture_position()

        tmp_dir = Path(tempfile.mkdtemp())
        conn = get_connection(tmp_dir / "discovered_check_classical.duckdb")
        init_schema(conn)
        position_ids = insert_positions(conn, [position])
        position["position_id"] = position_ids[0]
    ...
    @unittest.skipUnless(shutil.which("stockfish"), "Stockfish binary not on PATH")
    def test_classical_discovered_check_is_high_severity(self) -> None:
        settings = Settings(
            source="chesscom",
            chesscom_user="chesscom",
            chesscom_profile="classical",
        ...
        position = _discovered_check_classical_high_fixture_position()

        tmp_dir = Path(tempfile.mkdtemp())
        conn = get_connection(tmp_dir / "discovered_check_classical_high.duckdb")
        init_schema(conn)
        position_ids = insert_positions(conn, [position])
        position["position_id"] = position_ids[0]
        ...
        tactic_row, outcome_row = result
        self.assertEqual(tactic_row["motif"], "discovered_check")
        self.assertEqual(tactic_row["game_id"], "classical-discovered-check-high")
        self.assertGreaterEqual(tactic_row["severity"], 1.5)
        self.assertLessEqual(outcome_row["eval_delta"], -150)
        self.assertTrue(tactic_row["best_uci"])
    ...
    @unittest.skipUnless(shutil.which("stockfish"), "Stockfish binary not on PATH")
    def test_correspondence_discovered_check_is_low_severity(self) -> None:
        settings = Settings(
            source="chesscom",
            chesscom_user="chesscom",
        ...
        tmp_dir = Path(tempfile.mkdtemp())
        conn = get_connection(tmp_dir / "discovered_check_correspondence.duckdb")
        init_schema(conn)
        position_ids = insert_positions(conn, [position])
        position["position_id"] = position_ids[0]
    ...
    @unittest.skipUnless(shutil.which("stockfish"), "Stockfish binary not on PATH")
    def test_correspondence_discovered_check_is_high_severity(self) -> None:
        settings = Settings(
            source="chesscom",
            chesscom_user="chesscom",
            chesscom_profile="correspondence",
        ...
        tmp_dir = Path(tempfile.mkdtemp())
        conn = get_connection(tmp_dir / "discovered_check_correspondence_high.duckdb")
        init_schema(conn)
        position_ids = insert_positions(conn, [position])
        position["position_id"] = position_ids[0]
        ...
        tactic_row, outcome_row = result
        self.assertEqual(tactic_row["motif"], "discovered_check")
        self.assertEqual(tactic_row["game_id"], "correspondence-discovered-check-high")
        self.assertGreaterEqual(tactic_row["severity"], 1.5)
        self.assertLessEqual(outcome_row["eval_delta"], -150)
        self.assertTrue(tactic_row["best_uci"])
